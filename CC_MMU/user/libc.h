...
// request buffer value from process pid
extern void send(       int pid, int val);
extern int  getRequest(     int   pid);
extern int  receive(    int   pid); //Use this in user programs
extern int* memalloc(uint32_t size);
